...
if (typeof require !== "undefined") {
  ...
  var base = require("./base"); // eslint-disable-next-line no-redeclare
...
if (typeof require !== "undefined" && typeof XMLHttpRequest === "undefined") {
  var XMLHttpRequest = null; // eslint-disable-next-line camelcase
...
ripe.touchHandler = function (element, options) {
  ...
  var eventHandler = function eventHandler(event) {
    ...
    var first = touches[0];
    var type = ""; // switches over the type of touch event associating
    ...
    mouseEvent.initMouseEvent(type, true, true, window, 1, first.screenX, first.screenY, first.clientX, first.clientY, false, false, false, false, 0, null); // dispatches the event to the original target of the
...
if (typeof require !== "undefined") {
  ...
  require("./observable"); // eslint-disable-next-line no-redeclare
...
ripe.Ripe.prototype.init = function (brand, model, options) {
  ...
  this.ready = false; // extends the default options with the ones provided by the
  // developer upon this initializer call

  options = ripe.assign({
    options: false
  }, options); // runs the connfiguration operation on the current instance, using
  // the requested parameters and options, multiple configuration
  // operations may be executed over the object life-time

  this.config(brand, model, options); // listens for the post parts event and saves the current configuration
...
ripe.Ripe.prototype.config = function (brand, model, options) {
  ...
  this.brand = brand;
  this.model = model; // resets the history related values as the current
  // model has changed and no previous history is possible

  this.history = [];
  this.historyPointer = -1; // sets the new options using the current options
  ...
  this.setOptions(options); // determines if the defaults for the selected model should
  ...
  loadParts.call(this, function (result) {
    ...
    this.remote();
    this.update();
  }.bind(this)); // in case the current instance already contains configured parts
  ...
  this.ready = update ? this.ready : hasParts; // triggers the config event notifying any listener that the (base)
...
ripe.Ripe.prototype.setOptions = function (options) {
  ...
  this.useDiag = this.options.useDiag === undefined ? !this.noDiag : this.options.useDiag; // runs the background color normalization process that removes
...
ripe.Ripe.prototype.setParts = function (update, noUpdate, options) {
  if (_typeof(update) === "object" && !Array.isArray(update)) {
...
ripe.Ripe.prototype.undo = function () {
  ...
  parts && this.setParts(parts, false, {
    action: "undo"
  });
};
/**
...
ripe.animateProperty = function (element, property, initial, final, duration, callback) {
  ...
  var frame = function frame() {
    ...
    var animationDelta = timeDelta * (final - initial) / duration; // adjusts the value by the correspondent amount
    ...
    last = current; // checks if the animation has finished and if it is then
...
if (typeof require !== "undefined") {
  ...
  require("./ripe"); // eslint-disable-next-line no-redeclare


  var ripe = base.ripe; // eslint-disable-next-line no-redeclare
...
ripe.Ripe.prototype.getCombinationsP = function (options) {
  ...
  return new Promise(function (resolve, reject) {
    _this.getCombinations(options, function (result, isValid, request) {
      isValid ? resolve({
        result: result,
        request: request
      }) : reject();
...
ripe.Ripe.prototype._cacheURL = function (url, options, callback) {
  ...
  callback = typeof options === "function" ? options : callback;
  options = typeof options === "function" ? {} : options; // builds the (base) key value for the provided value
  // from options or used the default one

  var key = options.key || "default"; // creates the full key by adding the base key to the
  // URL value (including query string), this is unique
  // assuming no request payload

  var query = this._buildQuery(options.params || {});

  var fullKey = key + ":" + url + ":" + query; // determines if the current request should be cached, obeys
  // some of the basic rules for that behaviour

  var cached = typeof options.cached === "undefined" ? true : options.cached;
  cached = cached && !options.force && ["GET"].indexOf(options.method || "GET") !== -1; // initializes the cache object in the current instance
  // in case it does not exists already

  this._cache = this._cache === undefined ? {} : this._cache; // in case there's already a valid value in cache,
  ...
  return this._requestURL(url, options, function (result, isValid, request) {
    if (isValid && cached) {
...
ripe.Ripe.prototype._getCombinationsOptions = function (options) {
  ...
  var brand = options.brand === undefined ? this.brand : options.brand;
  var model = options.model === undefined ? this.model : options.model;
  var useName = options.useName !== undefined && options.useName !== null ? options.useName : false;
  var country = options.country === undefined ? this.country : options.country;
  var flag = options.flag === undefined ? this.flag : options.flag;
...
ripe.Ripe.prototype.auth = function (username, password, options, callback) {
  callback = typeof options === "function" ? options : callback;
  options = typeof options === "function" ? {} : options;
  this.signin(username, password, options, function (result) {
    this.sid = result.sid;
    this.trigger("auth");
...
ripe.Ripe.prototype.oauth = function (options, callback) {
  ...
  if (oauthToken && clientId && clientSecret && redirectUri) {
    return this.oauthLogin(oauthToken, options, function (result, isValid) {
      if (isValid && result) {
        this.sid = result.sid;
        this.trigger("auth");
        callback && callback(result);
  ...
  if (code && clientId && clientSecret && redirectUri) {
    return this.oauthAccessToken(code, {
      clientId: clientId,
      clientSecret: clientSecret,
      redirectUri: redirectUri
...
ripe.Ripe.prototype.oauthRedirect = function (options, callback) {
  ...
  var currentUrl = location.protocol + "//" + location.host + "/" + location.pathname.split("/")[1];
...
ripe.Ripe.plugins.DiagPlugin.prototype._setHeaders = function (request) {
  request.setRequestHeader("X-Ripe-Sdk-Version", "1.4.10");
  var plugins = [];
  var index = null;
  ...
  pluginsS && request.setRequestHeader("X-Ripe-Sdk-Plugins", pluginsS); // in case the brand value is defined in the owner it is also added
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype.register = function (owner) {
  ...
  this.owner.getConfig({}, function (config) {
    ...
    this.optionals = optionals; // binds to the pre parts event so that the parts can be
    // changed so that they comply with the product's restrictions

    this.owner.bind("part", this.partCallback); // resets the current selection to trigger
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._applyRestrictions = function (name, value) {
  ...
  var partSet = name !== undefined ? {
    name: name,
    material: value.material,
    color: value.color
  } : null;
  name !== undefined && customization.push(partSet); // obtains the new parts and mutates the original
  ...
  for (var index = 0; index < newParts.length; index++) {
    ...
    var oldPart = this.owner.parts[newPart.name]; // if a change was made due to the restrictions
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._solveRestrictions = function (availableParts, restrictions, customization, solution) {
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._getRestrictionKey = function (part, material, color, token) {
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._buildRestrictionsMap = function (restrictions) {
  var restrictionsMap = {}; // iterates over the complete set of restrictions in the restrictions
  ...
  for (var index = 0; index < restrictions.length; index++) {
    ...
    if (restriction.length === 1) {
      ...
      var key = this._getRestrictionKey(_restriction.part, _restriction.material, _restriction.color);
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._isRestricted = function (newPart, restrictions, parts) {
  ...
  keyRestrictions = materialRestrictions instanceof Array ? keyRestrictions.concat(materialRestrictions) : keyRestrictions;
  keyRestrictions = colorRestrictions instanceof Array ? keyRestrictions.concat(colorRestrictions) : keyRestrictions;
  ...
  for (var index = 0; index < keyRestrictions.length; index++) {
    ...
    for (var _index = 0; _index < parts.length; _index++) {
      ...
      var restrictionKeys = [this._getRestrictionKey(null, part.material), this._getRestrictionKey(null, null, part.color), this._getRestrictionKey(null, part.material, part.color)];
...
ripe.Ripe.plugins.RestrictionsPlugin.prototype._alternativeFor = function (newPart, availableParts, pop) {
  pop = pop || false;
  var part = null;
  var color = null;
  var colors = null;
  var materialsIndex = null; // finds the index of the part to use it as the starting
...
ripe.Ripe.plugins.SyncPlugin.prototype.register = function (owner) {
  ripe.Ripe.plugins.Plugin.prototype.register.call(this, owner); // binds to the part event to change the necessary parts
  // so that they comply with the product's sync rules

  this.owner.bind("part", this.partCallback); // resets the current selection to trigger the sync operation
...
ripe.Ripe.plugins.SyncPlugin.prototype._applySync = function (name, value) {
  for (var key in this.rules) {
    ...
    value = value || this.owner.parts[name]; // checks if the part triggers the sync rule
...
if (typeof require !== "undefined") {
  ...
  require("./visual"); // eslint-disable-next-line no-redeclare
...
ripe.Configurator.prototype.init = function () {
  ...
  this.owner.bind("deselected_part", function (part) {
    this.lowlight();
  }.bind(this)); // creates a structure the store the last presented
  // position of each view, to be used when returning
  // to a view for better user experience

  this._lastFrame = {}; // ues the owner to retrieve the complete set of frames
...
ripe.Configurator.prototype.update = function (state, options) {
  ...
  var preload = options.preload; // checks if the parts drawed on the target have
  ...
  this.signature = signature; // if the parts and the position haven't changed
  ...
  this.unique = unique; // runs the load operation for the current frame, taking into
  ...
  this._loadFrame(view, position, {
    ...
    animate: animate,
    duration: duration
  }, callback); // runs the pre-loading process so that the remaining frames are
...
ripe.Configurator.prototype.changeFrame = function (frame, options) {
  ...
  this.lowlight(); // saves the position of the current view
  // so that it returns to the same position
  // when coming back to the same view

  this._lastFrame[view] = position;
  this.element.dataset.position = nextPosition; // if there is a new view and the product supports
  ...
  if (duration) {
    animate = type || animate; // calculates the number of steps of
    ...
    options.duration = duration - stepDuration; // determines the next step and sets it
    // as the position
  ...
  var animated = Boolean(duration); // if the frame change is animated and preventDrag is true
...
ripe.Configurator.prototype.highlight = function (part, options) {
  ...
  var self = this; // runs the default operation for the parameters that this
  // function receives

  options = options || {}; // determines the current position of the configurator so that
  ...
  var maskDuration = this.element.dataset.mask_duration || this.maskDuration; // adds the highlight class to the current target configurator meaning
  // that the front mask is currently active and showing info

  this.element.classList.add("highlight"); // constructs the full URL of the mask image that is going to be
  ...
  var frontMaskLoad = function frontMaskLoad() {
...
ripe.Configurator.prototype._initLayout = function () {
  ...
  this.element.classList.add("configurator"); // creates the area canvas and adds it to the element

  var area = ripe.createElement("canvas", "area");
  var context = area.getContext("2d");
  context.globalCompositeOperation = "multiply";
  this.element.appendChild(area); // adds the front mask element to the element,
  // this will be used to highlight parts

  var frontMask = ripe.createElement("img", "front-mask");
  this.element.appendChild(frontMask); // creates the back canvas and adds it to the element,
  ...
  this.element.appendChild(back); // creates the mask element that will de used to display
  // the mask on top of an highlighted or selected part

  var mask = ripe.createElement("canvas", "mask");
  this.element.appendChild(mask); // adds the framesBuffer placeholder element that will be used to
  // temporarily store the images of the product's frames

  var framesBuffer = ripe.createElement("div", "frames-buffer"); // creates a masksBuffer element that will be used to store the various
  ...
  this._populateBuffers(); // set the size of area, frontMask, back and mask


  this.resize(); // sets the initial view and position

  this.element.dataset.view = this.view;
  this.element.dataset.position = this.position; // register for all the necessary DOM events
...
ripe.Configurator.prototype._updateConfig = function () {
  ...
  // is being loaded
  this.ready = false; // updates the parts list for the new product

  this._initPartsList(); // retrieves the new product frames and sets them


  this.owner.getFrames(function (frames) {
    this.frames = frames; // tries to keep the current view and position
...
ripe.Configurator.prototype._loadFrame = function (view, position, options, callback) {
  ...
  image = image || front; // in case there's no images for the frames that are meant
  ...
  this._loadMask(maskImage, view, position, options); // builds the URL that will be set on the image
  ...
  }.bind(this); // sets the src of the image to trigger the request
...
ripe.Configurator.prototype._preload = function (useChain) {
  ...
  index++;
  this.index = index;
  this.element.classList.add("preload"); // adds all the frames to the work pile
  ...
  var mark = function mark(element) {
    ...
    var pending = framesBuffer.querySelectorAll("img.preloading") || []; // if there are images preloading then adds the
  ...
  var render = function render() {
    ...
    reference.classList.add("preloading"); // if a chain base loaded is used then
    // marks the current frame as pre-loaded
    // and proceeds to the next frame

    var callbackChain = function callbackChain() {
      mark(reference);
      render();
    ...
    var callbackMark = function callbackMark() {
      mark(reference);
...
ripe.Configurator.prototype._registerHandlers = function () {
  ...
  var self = this; // retrieves the reference to the multiple elements that
  // are going to be used for event handler operations

  var area = this.element.querySelector(".area");
  var back = this.element.querySelector(".back"); // registes for the selected part event on the owner
  // so that we can highlight the associated part

  this.owner.bind("selected_part", function (part) {
    this.highlight(part);
  }.bind(this)); // binds the mousedown event on the element to prepare
  ...
  area.addEventListener("mousemove", function (event) {
    ...
    var index = self._getCanvasIndex(this, event.offsetX, event.offsetY); // in case the index that was found is the zero one this is a special
...
ripe.Configurator.prototype._parseDrag = function () {
  ...
  var verticalThreshold = this.element.dataset.verticalThreshold || this.verticalThreshold; // if the drag was vertical then alters the
  ...
  nextPosition = nextPosition >= 0 ? nextPosition : viewFrames + nextPosition; // if the view changes then uses the last
...
ripe.Image.prototype._registerHandlers = function () {
  ...
  this.element.addEventListener("load", this.loadListener); // eslint-disable-next-line no-undef

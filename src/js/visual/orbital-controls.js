...
ripe.OrbitalControls = function(configurator, owner, element, options) {
...
ripe.OrbitalControls.prototype.updateRotation = function(frame, options) {
    ...
    else this._checkViewPositionRotations(frame, options);
};
...
ripe.OrbitalControls.prototype._updateDragRotations = function() {
    ...
    if (this._baseVerticalRot + this.mouseDeltaY !== this.currentVerticalRot) {
        if (this.mouseDeltaY >= this.maximumVerticalRot - this._baseVerticalRot) {
            diff = this.mouseDeltaY - (this.maximumVerticalRot - this._baseVerticalRot);

            this.referenceY -= diff;
            this.mouseDeltaY += diff;
        } else if (this.mouseDeltaY <= this.minimumVerticalRot - this._baseVerticalRot) {
    ...
    if (needsUpdate) this.configurator.applyRotations();
};
...
ripe.OrbitalControls.prototype._checkViewPositionRotations = function(frame, options) {
    ...
    console.log(position, nextView, view, nextView, animating);
    ...
    */
    ...
    const rotationTransition = () => {
        ...
        this.currentHorizontalRot += stepX;
        this.currentVerticalRot += stepY;

        this.configurator.applyRotations();
    ...
    if (view !== nextView) {
        ...
        stepY = (finalRotation - this._baseVerticalRot) / duration;
    }
    if (position !== nextPosition) {
        finalRotation = this._positionToRotation(nextPosition);

        if (this._baseHorizontalRot + 180 > finalRotation) {
            stepX = (finalRotation - this._baseHorizontalRot) / duration;
        } else {
            stepX = (this._baseHorizontalRot - finalRotation) / duration;
        }
    }

...
/**
 * Server side callback method to calculate the maximum allowed initials
 * for a certain customization/personalization context.
 * This method allows the change of the current context of execution based on
 * a server side implementation of the build's business logic.
 *
 ...
 * @param {Function} callback Function with the result of the request.
 * @returns {XMLHttpRequest} Resulting information for the callback execution,
 * meaning the the maximum allowed initials.
 */
ripe.Ripe.prototype.maximumInitials = function(options, callback) {
    callback = typeof options === "function" ? options : callback;
    options = typeof options === "function" || options === undefined ? {} : options;
    options = this._maximumInitialsOptions(options);
    options = this._build(options);
    return this._cacheURL(options.url, options, callback);
};

/**
 * Server side callback method to calculate the maximum allowd initials
 * for a certain customization/personalization context.
 * This method allows the change of the current context of execution based on
 * a server side implementation of the build's business logic.
 *
 ...
 * in case of success the maximum allowed initials.
 */
ripe.Ripe.prototype.maximumInitialsP = function(options, callback) {
    return new Promise((resolve, reject) => {
        this.maximumInitials(options, (result, isValid, request) => {
            isValid ? resolve(result) : reject(new ripe.RemoteError(request));
        });
    });
};
...
/**
 * @ignore
 * @see {link http://docs.platforme.com/#product-endpoints-maximum_characters}
 */
ripe.Ripe.prototype._maximumInitialsOptions = function(options = {}) {
    const brand = options.brand === undefined ? this.brand : options.brand;
    const model = options.model === undefined ? this.model : options.model;
    const initials = options.initials === undefined ? this.initialsExtra : options.initials;
    const parts = options.parts === undefined ? this.parts : options.parts;
    const choices = options.choices === undefined ? this.choices : options.choices;
    const group = options.group === undefined ? null : options.group;
    const url = this.url + "brands/" + brand + "/models/" + model + "/logic/maximum_initials";
    const ctx = Object.assign({}, this.ctx || {}, {
        brand: brand,
        model: model,
        initials: initials,
